...
def apply_experience(main_model, target_model,experience, experience_errors):
    ...
    sample_inds = np.random.choice(range(len(experience)), experience_sample_size, p = weights)
...
def q_learning_predict_bb(x,model):

    bb_history = []

    count = 0

    (width, height, d) = x.shape
    initial_history = [-1] * history_length
    initial_bb = (0, 0, height, width)
    s = State(initial_history, initial_bb, x)
    ...
    for i in range(sys.maxsize):
        ...
        if a == 8:
            ...
            historyp.append(a)
            assert len(historyp) == history_length
            sp = State(historyp, bbp, x)
            s = sp
        if done:
            break

    return s.history, bb_history
